...
void main() {
  ...
  group(OnGetContactPressedUsecase, () {
    test(
      'call',
      () async {
        var usecase = OnGetContactPressedUsecase().call(1);
        expect(usecase.name.toString(), 'login');
        expect(usecase.params, equals({'redirect': 'true'}));
        expect(usecase.query, equals({'fromTitle': detailsPageTitle}));
      },
    );
  });
  // group(
